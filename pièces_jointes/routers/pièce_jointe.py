...
@router.post('/analyze_file', status_code=201)
def analyze_file(path: FilePath):
    malicious=False
    id = upload_file(path.path)
    report = get_report(id)
    if report:
        print("\nRésultat de l'analyse :")
        for engine, result in report.items():
            if (result['category']=="malicious"):
                malicious=True
            print(f"{engine}: {result['category']} - {result['result']}")
    return {"message": malicious}